...
- Status: PROMOVIDO COM GATES ABERTOS; ver o registro de execução abaixo. O plano continua autoritativo e ainda não está aprovado para DONE.
...
- Esta rodada fecha a prova de alocação e prefill agregado no teto configurado, mas não fecha o gate de teto útil para conversas que precisem de mais de um token de saída, nem os gates de 20 ciclos, profiling completo, comparação com o router, cobertura API/concorrência e pytest formal.
...
- O `master` incorporou o upstream atual `origin/master` em `093a2f86c3e37c54fa3e1f9efb17b304f3433abd`, preservando a implementação adaptativa e resolvendo conflitos somente em `src/CMakeLists.txt`, `tests/CMakeLists.txt`, `tools/server/server-models.cpp` e `tools/server/server-models.h`. O merge da integração é `fe3c817f2`; o commit de código usado no build é `06a245d5e0acee5f94156754215c78a065a99d17`. Os commits posteriores até o HEAD final `68e659890` alteraram somente este plano/documentação, portanto não mudam o binário.
...
### Reprodução A/B e correção de CUDA Graph — 14/09/2026

- A reprodução foi feita na GOKAYA com a produção parada, o mesmo GGUF Qwen3.8-27B, `CUDA0`, `ctx-size=97536`, `ctx-size-mtp=56320`, `mtp-max-tokens=56320`, `parallel=1`, `batch/ubatch=256`, KV `q4_0`, `fit=off`, MTP e os demais switches da unit. O harness construiu exatamente os prompts de `8.806`, `10.598`, `3.676`, `11.942` e `56` tokens e usou `n_predict=1024` nos quatro primeiros, `ignore_eos=true`.
- O binário anterior `/home/hjotha/worktrees/llama-adaptive-context-impl/build-adaptive-cuda/bin/llama-server` passou os cinco requests na porta `19429`, HTTP `200`, `1024/1024/1024/1024/1` tokens e `server_rc=0` (`gauntlet/oom-ab-old-decode.{json,log}`). O binário novo integrado CUDA+Vulkan `609bda686506a347361a276826a6f0a6c095bded246a2fa7edf85a3ed7120806` reproduziu o OOM após os dois primeiros requests na porta `19430`: terceiro request desconectado, GPU em `11901 MiB` usados/`3 MiB` livres e `server_rc=-6` (`gauntlet/oom-ab-new-graph.{json,log}`).
- O mesmo binário novo com `GGML_CUDA_DISABLE_GRAPHS=1` passou os cinco requests na porta `19431`, com `server_rc=0` (`gauntlet/oom-ab-new-nograph.{json,log}`). Isso separa a regressão do caminho CUDA Graph da execução direta; os prompts, limites e modelo foram mantidos.
- A causa operacional é a instância executável/captura CUDA Graph consumindo a margem já pequena da 4070. O caminho existente só tratava OOM em `cudaGraphInstantiate`; não verificava headroom antes de capturar/recapturar e mantinha instâncias vivas quando caía para execução direta. O `MUL_MAT` assíncrono então chegava a `cudaGetLastError()` com `cudaErrorMemoryAllocation`, e `CUDA_CHECK` abortava o processo.
- Correção mínima no master: `ggml_backend_cuda_context` centraliza o limiar `GGML_CUDA_GRAPH_RECOVERY_HEADROOM_MB` (default seguro `32 MiB`), consulta `cudaMemGetInfo()` antes de habilitar/capturar, sincroniza e destrói `cudaGraphExec_t`/`cudaGraph_t` sob pressão, zera warmup/UID e mantém a recuperação automática quando a folga volta. O caminho direto não consulta novamente o mapa de graphs no fim da avaliação. Não há redução de `ctx-size`, reload do modelo ou desligamento global permanente de graphs.
- O build da fonte corrigida foi configurado em `/home/hjotha/releases/llama-adaptive-fix-20260914/build-cuda-vulkan` como `Release` (`-O3 -DNDEBUG`), CUDA Toolkit `13.3.73`, `89-real`, `GGML_CUDA=ON`, `GGML_CUDA_GRAPHS=ON`, FlashAttention, NCCL e `GGML_VULKAN=ON`; terminou `exit=0` com `-j8`. `test-arg-parser` terminou `test-arg-parser: all tests OK` e enumerou a RTX 4070 e os dois dispositivos Vulkan. O binário corrigido tem SHA-256 `980f6b62e808a4518a89a6908fb6a4000de492b1327c47f6a7ebc1ed39790db3` e linka `libggml-cuda.so`, `libggml-vulkan.so`, CUDA 13 e Vulkan.
- O mesmo estresse do binário corrigido passou na porta `19432`: cinco HTTP `200`, tokens `1024/1024/1024/1024/1`, `server_rc=0`, GPU estabilizada em `11883 MiB` usados/`21 MiB` livres. O log registra apenas desativações locais antes da captura em `30.06 MiB` e `24.06 MiB` livres abaixo do limiar de `32 MiB`; não há `out of memory`, erro CUDA, Xid, SIGABRT, core ou segmentation fault (`gauntlet/oom-ab-fix.{json,log}`).
- O gate de produção ainda requer revisão adversarial Sol desta correção, smoke funcional e verificação final de unit/PID/journal. A validação atual prova a regressão e a recuperação no workload que reproduzia o crash; não transforma os gates históricos de concorrência ampla, profiling físico, comparação com router e sanitizers em concluídos.

### Desativação Vulkan por processo no servidor CUDA — 14/09/2026

- A inspeção do build unificado confirmou que `ggml_backend_vk_reg()` chamava `ggml_vk_instance_init()` durante o registro do backend. Essa inicialização enumerava a Radeon 780M e a RTX 4070 antes de qualquer dispositivo Vulkan ser escolhido; no processo Qwen3.8-27B isso consumia aproximadamente `12 MiB` de VRAM estática na 4070, reduzindo a folga disponível para CUDA Graph.
- A correção mantém `GGML_VULKAN=ON` no binário unificado, mas fecha a rota estática e a rota de plugin: `ggml_backend_vk_reg()` retorna `nullptr` antes de `ggml_vk_instance_init()` quando `GGML_DISABLE_VULKAN` está presente, e `ggml_backend_vk_reg_get_device_count()` retorna zero como defesa adicional. O registro estático já respeitava a mesma variável; agora um backend Vulkan carregado dinamicamente também não pode criar a instância. O caminho sem a variável não mudou.
- O rebuild remoto Release CUDA+Vulkan em `/home/hjotha/releases/llama-adaptive-fix-20260914/build-cuda-vulkan` terminou com exit `0` (`-j8`), nos alvos `llama-server` e `test-arg-parser`. O binário continua linkado a `libggml-cuda.so`, `libggml-vulkan.so`, CUDA 13 e Vulkan; SHA-256 verificado: `980f6b62e808a4518a89a6908fb6a4000de492b1327c47f6a7ebc1ed39790db3`.
- `test-arg-parser` sem a variável terminou `test-arg-parser: all tests OK` e enumerou os dois dispositivos Vulkan (AMD e RTX), provando que o suporte 8092 continua disponível. Com `GGML_DISABLE_VULKAN=1`, o mesmo teste terminou `test-arg-parser: all tests OK`, inicializou somente CUDA e registrou `Vulkan backend disabled by GGML_DISABLE_VULKAN environment variable`, sem `ggml_vulkan: Found`.
- O A/B real na GOKAYA, porta efêmera `19446`, usou o mesmo GGUF, switches, prompts `8.806/10.598/3.676/11.942/56`, `n_predict=1024` nos quatro primeiros e `parallel=1`, com `GGML_DISABLE_VULKAN=1`, `GGML_CUDA_GRAPH_RECOVERY_HEADROOM_MB=18` e debug de graphs. Foram `5/5` HTTP 200, tokens `1024/1024/1024/1024/1`, `server_rc=0`; o log mostra captura/instanciação CUDA, desativação preventiva em `17,56 MiB < 18 MiB`, liberação de `1` executável e `1` captura, e nenhuma ocorrência de OOM, Xid, erro CUDA, abort ou segmentation fault. O baseline após carregar o modelo foi `11.853 MiB` usados/`50 MiB` livres, contra `11.865 MiB`/`39 MiB` com Vulkan habilitado: `12 MiB` recuperados antes das alocações do request.
- A unit de produção CUDA 27B deve exportar `GGML_DISABLE_VULKAN=1` junto com o headroom de `18 MiB`; a unit Vulkan 4B da porta 8092 não deve exportar essa variável. O build HIP/ROCm foi interrompido por decisão explícita do usuário e não faz parte deste alvo CUDA+Vulkan.
- A revisão Sol persistente desta fatia respondeu `ACCEPT_WITH_NOTES` em `gauntlet/vulkan-disable-sol-review.md` e `.json`, sem blocker para commit ou promoção no perfil CUDA com a variável fixa no startup. A nota não bloqueante é que `GGML_BACKEND_DL=ON` não foi testado especificamente; o contrato do loader, a semântica por presença (`GGML_DISABLE_VULKAN=0` também desativa) e a exigência de definir a variável antes da construção do registry ficaram registrados. A unit já foi instalada, e o smoke em 8090 confirmou PID, listeners, `/health`, `/props`, journal e ausência de enumeração Vulkan. Os gates históricos de profiling físico, comparação com router, concorrência ampla e rollback continuam abertos.

### Promoção final CUDA com Vulkan runtime-disabled — 14/09/2026

- A unit auditável está em `gauntlet/production/llama-server-root.service.final` e foi validada no host .57 com `systemd-analyze verify`. Ela aponta para `/home/hjotha/releases/llama-adaptive-fix-20260914/build-cuda-vulkan/bin/llama-server`, exporta `GGML_DISABLE_VULKAN=1` e `GGML_CUDA_GRAPH_RECOVERY_HEADROOM_MB=18`, mantém `--device CUDA0`, `--ctx-size 97536`, `--ctx-size-mtp 56320`, `--mtp-max-tokens 56320` e `--parallel 1`. A unit anterior foi preservada em `/etc/systemd/system/llama-server-root.service.pre-vulkan-disable-20260914`.
- A unit foi instalada com `daemon-reload` e habilitada. O estado verificado depois do start é `ActiveState=active`, `SubState=running`, `MainPID=1497943`, `ExecMainStatus=0`, `NRestarts=0`, listener `0.0.0.0:8090`; a unit Vulkan 4B 8092 não foi alterada e permanece sem listener. O ambiente efetivo publicado pelo systemd contém a variável de desativação e o headroom de 18 MiB.
- Após a carga de aproximadamente 44 s, `/health` retornou HTTP 200 e `/props` retornou HTTP 200 com `enabled=true`, `profile=mtp`, `state=ready`, `context_size=56320`, `context_size_long=97536` e `mtp_weights_resident=true`. A completion real em 8090 retornou HTTP 200, `tokens_predicted=1` e `tokens_evaluated=6`; o relatório completo está em `gauntlet/production/production-final-20260914.txt`.
- O journal desde o start não contém `Vulkan backend`, `ggml_vulkan: Found`, `out of memory`, `Xid`, `cuda error`, `SIGABRT`, `abort` ou `segmentation`. O processo ativo continua usando o binário corrigido com SHA-256 `980f6b62e808a4518a89a6908fb6a4000de492b1327c47f6a7ebc1ed39790db3` e os requests externos observados não produziram restart (`NRestarts=0`).
- A promoção fecha a validação operacional do perfil CUDA com a instância Vulkan suprimida. O smoke Vulkan0 independente do mesmo binário continua aprovado em `gauntlet/vulkan-4b-smoke-19447`, com Qwen3.5-4B: health 200, completion 200, `tokens_predicted=1` e encerramento limpo. O build HIP/ROCm segue deliberadamente interrompido e fora do alvo.
- O plano permanece em implementação, não DONE: continuam abertos profiling físico detalhado, comparação quantitativa com o router, concorrência/múltiplos slots, sanitizers adicionais, persistência ampla de slots e gates de rollback/soak. A promoção atual é o binário CUDA+Vulkan unificado com desativação Vulkan somente no processo 27B.
- A revisão final do diff completo respondeu `ACCEPT_WITH_NOTES` em `gauntlet/final-sol-review-vulkan.md` e `.json`, sem blocker concreto. O Sol confirmou a unit ativa, hash do binário, health/props/completion, ausência de Vulkan/OOM/Xid/abort no journal e o tráfego longo real após a promoção. Permanecem somente as notas de `GGML_BACKEND_DL=ON` não exercitado, semântica da variável por presença/startup, perfil validado serial (`parallel=1`, headroom 18 MiB) e soak/concorrência ampla ainda não executados.

### Migração do desenvolvimento para a GOKAYA — 14/09/2026

- O fork `hjotha/llama.cpp` incorporou o `origin/master` oficial em `cd81da5508955ed9b68113a5de62d77c77db3ba9` e esse commit foi publicado em `fork/master`. As duas exclusões locais preexistentes (`CLAUDE.md` e `benches/dgx-spark/run-aime-120b-t8-x8-high.log`) foram restauradas antes do push e permanecem fora do histórico.
- O checkout de desenvolvimento passou a ser `/home/hjotha/llama` na GOKAYA `.57` (`192.168.1.57`), com `origin` apontando para o fork e o remoto oficial preservado como `upstream`. A compatibilidade do pacote de transferência antigo é mantida por um symlink para esse checkout.
- A regra global da `.57` foi complementada: tarefas de desenvolvimento de `llama.cpp` nesse checkout editam diretamente o master/branches normais e não criam worktrees vinculadas, preservando alterações alheias e exigindo validação antes de commit. Nenhuma sessão Codex existente foi iniciada, encerrada ou movida.
- As cinco worktrees locais do llama estavam com os commits já contidos no master; as cópias sujas eram snapshots regressivos e não foram mescladas cegamente. Patches, estados e evidências seletivas foram copiados, com SHA-256 conferido, em `/home/hjotha/llama/archives/optiplex-migration-20260914/` na GOKAYA. O pacote inclui também os artefatos auxiliares e o repositório `llama-metrics-dash` para continuidade remota.
- Após a verificação remota, o checkout, worktrees e artefatos de desenvolvimento do llama no optiplex serão removidos por `gio trash`. Repositórios operacionais não relacionados e os transcripts permanecerão intactos. O build final e a produção continuam sendo responsabilidade da `.57`; este registro não marca o plano como DONE.
