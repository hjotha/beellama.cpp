...
The source also supports optional fourth/fifth profiles, `xlong` and `xxlong`, through `--ctx-size-xlong` / `--ctx-size-xxlong` and `--xlong-max-tokens` / `--xxlong-max-tokens`. Their context sizes and thresholds must follow the preceding tiers. LONG and XL may keep MTP enabled with `--spec-draft-n-max-long N` / `--spec-draft-n-max-xlong N`; XXL remains target-only (`--spec-draft-n-max-xxlong` must be zero). The resident MTP draft KV cache can be selected per tier with `--spec-draft-type-k-s/-m/-l/-xl/-xxl` (and the `-v` counterparts), defaulting to the global `--spec-draft-type-k`/`--spec-draft-type-v` — e.g. a compact KVarN draft on XL while S/M/L keep q4_0. Each has independent batch/ubatch controls, such as `--batch-size-xlong` and `--ubatch-size-xlong`, to reduce workspace at larger contexts. Short aliases `--ctx-size-s`, `--ctx-size-m`, `--ctx-size-l`, `--ctx-size-xl` and `--ctx-size-xxl` are available. Long/XL/XXL also expose per-profile K/V type controls, such as `--cache-type-k-xlong` and `--cache-type-v-xlong`; representation changes remain subject to restore compatibility. These options do not establish a hardware-independent context ceiling.
